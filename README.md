...
[![Standards](https://img.shields.io/badge/standards-ISO%2027001%20%7C%20GDPR%20%7C%20UU%20PDP-green.svg)](compliance/README.md)
...
├── .gitignore
├── compliance/
│   ├── FULL-PROJECT-SCAN-METHODOLOGY.md    ← binding 7-phase procedure (the agent's brain)
│   ├── README.md                          ← 31-commitment ADR matrix (the verification baseline)
│   └── references/
│       ├── gdpr-celex-32016R0679-en.md    ← GDPR source text
│       └── iso27001-2022-annex-a-93-controls.md ← Annex A 93-control reference
├── prompts/
│   └── scan-agent-prompt.md               ← copy-paste invocation wrapper (+ worked example)
└── templates/
...
# 2. Freeze the target's evidence point
git -C /path/to/<target-repo> rev-parse HEAD
git -C /path/to/<target-repo> status --porcelain

# 3. Fill the 12 variables in prompts/scan-agent-prompt.md, paste the
#    block into your agent along with the files in "Attachments",
#    and let it execute compliance/FULL-PROJECT-SCAN-METHODOLOGY.md.
```

First run should take one directory + two commitments as a trial, then
expand to full scope (see `prompts/scan-agent-prompt.md → Tips`).
...
2. **Fill every variable.** The 12 placeholders (`<TARGET_REPO_URL>`,
...
[`prompts/scan-agent-prompt.md`](prompts/scan-agent-prompt.md#worked-example-illustrative-values--replace-all-of-them).
...
Semantic versioning (`CHANGELOG.md`). Current release: **v1.0.0** —
initial open-source release: generic methodology, generalized ADR matrix,
report template, agent prompt, bundled references.
